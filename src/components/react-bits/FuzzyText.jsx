  ...
  useEffect(() => {
    ...
    const init = async () => {
      ...
      if (typeof fontSize === 'number') {
        computedSize = `${fontSize}px`;
      } else if (fontSize === 'inherit') {
        computedSize = window.getComputedStyle(canvas.parentElement).fontSize;
      } else {
      ...
      lastComputedSize = computedSize;
      const numericFontSize = parseFloat(computedSize) || 32;
      computedSize = `${numericFontSize}px`;

      const fontString = `${fontWeight} ${computedSize} ${computedFontFamily}`;
      ...
      const finalColor = color === 'inherit' || color.startsWith('var') ? window.getComputedStyle(canvas).color : color;
    ...
    // Check for font size changes (CSS late injection)
    resizeObserver = new ResizeObserver(() => {
      if (!canvas || !canvas.parentElement) return;
      const newSize = window.getComputedStyle(canvas.parentElement).fontSize;
